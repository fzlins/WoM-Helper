...
- **Quest collect-all** — On the [/quests](https://minesweeper.online/quests) page, a button (mirroring the site’s own claim button style and label) is injected into the last column header of each quest table whenever the table has collectable rows. Clicking it claims all available rewards in that table at once. The button disappears automatically once all rewards have been claimed.- **Sell max button** — On the [/marketplace](https://minesweeper.online/marketplace) page, a ▲ link is injected after each quantity input in the Sell modal, and another in the quantity column header. Clicking a row link fills that input with the maximum quantity (the amount you own); clicking the header link fills all inputs at once.- **Dynamic content** — Uses a `MutationObserver` to handle content loaded after the initial page render.
